...
const VehicleScene=forwardRef<SceneHandle,Props>(function VehicleScene(props,ref){
 ...
 useEffect(()=>{
  ...
  new GLTFLoader().load(vehicle.model,gltf=>{
  ...
  function fitView(immediate=false,dt=1/60){
   ...
   controls.maxDistance=Math.max(180,fullDistance*1.2);
  ...
  function frame(now:number){raf=requestAnimationFrame(frame);const dt=Math.min((now-last)/1000,.05);last=now;if(document.hidden)return;
   ...
   if(geometryChanged){
    ...
    parts.forEach(({id})=>{const g=groups[id],o=offsets[id];g.position.set(o[0]*amount*(1-individual),o[1]*amount*(1-individual),o[2]*amount*(1-individual));g.visible=!p.isolated||p.selected===id;
     if(vehicle.illustrative.includes(id))g.visible=g.visible&&(amount>.08||p.isolated)&&(individual<.98||p.isolated);
    });
   ...
   if(now-lastLabels>50||propsChanged||invalidated){
    ...
    labelNodes.forEach(({b,id})=>{const show=individual<.5&&readyRef.current&&p.labels&&(!vehicle.illustrative.includes(id)||amount>.08||p.isolated)&&(!p.isolated||p.selected===id);
     if(b.hidden===show)b.hidden=!show;if(!show)return;
     const a=anchors[id];if(vehicle.id==='model-x')vector.set(...a).add(groups[id].position);else new THREE.Box3().setFromObject(groups[id]).getCenter(vector);vector.project(camera);b.style.display=vector.z<1?'flex':'none';b.classList.toggle('chosen',id===p.selected);
 ...
 return <><div ref={host} className="canvas-host" aria-label="Rotatable exploded 3D vehicle model"/>{!ready&&!error&&<div className="scene-loading"><span/>Loading {props.vehicle.make} {props.vehicle.name}…</div>}{error&&<div className="scene-error"><h3>The 3D view needs a moment.</h3><p>{error}</p><button onClick={()=>location.reload()}>Reload the view</button></div>}</>;
});
