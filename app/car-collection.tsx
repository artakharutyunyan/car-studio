...
export default function CarCollection({category='road'}:{category?:'road'|'formula-1'}) {
  ...
  return <main className="garage">
    ...
    <section className="garage-collection" aria-labelledby="collection-title">
      ...
      <div className="collection-search"><Search size={16}/><input type="search" value={query} onChange={e=>setQuery(e.target.value)} placeholder={racing?'Search Formula 1 cars…':'Search road cars…'} aria-label="Search cars"/>{query&&<button type="button" onClick={()=>setQuery('')} aria-label="Clear search"><X size={15}/></button>}</div>
