...
import {getVehicle} from '../app/vehicles';
import '../app/globals.css';

const match=window.location.pathname.match(/^\/cars\/([^/]+)\/?$/);
const car=match?getVehicle(match[1]):undefined;
const page=window.location.pathname==='/'?<Garage/>:window.location.pathname.replace(/\/$/,'')==='/formula-1'?<FormulaOne/>:car?<CarStudio key={car.id} vehicle={car}/>:<main className="garage"><h1>Car not found</h1><a href="/">Back to the collection</a></main>;
