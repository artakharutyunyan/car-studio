...
An interactive collection of seventy-eight road cars and thirty-three Formula 1 cars. Choose a card, orbit the model, isolate a component or mesh piece, and use the explosion slider to separate its geometry. Both collections can be searched or browsed alphabetically by make and model, and previous/next buttons on each car's detail page step through the same alphabetical order without returning to the collection grid.
